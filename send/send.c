...
ConnectToServer(const char *hostname, int port)
{
  ...
  */
  ...
  p+=sprintf(p,"Content-Type: text/xml; charset=\"utf-8\"\r\n");
  p+=sprintf(p,"SOAPACTION: \"urn:av-openhome-org:service:Spotify:1#Play\"\r\n");
  p+=sprintf(p,"Content-Length: 255\r\n");//很重要
  p+=sprintf(p,"Connection: close\r\n");
  p+=sprintf(p,"Host: 192.168.1.113:33198\r\n");
  p+=sprintf(p,"\n\r");
  p+=sprintf(p,"<s:Envelope s:encodingStyle=\"http://schemas.xmlsoap.org/soap/encoding/\" xmlns:s=\"http://schemas.xmlsoap.org/soap/envelope/\"><s:Body><u:Play xmlns:u=\"urn:av-openhome-org:service:Spotify:1\"><Url>changpeng</Url><Index>2</Index></u:Play></s:Body></s:Envelope>");
